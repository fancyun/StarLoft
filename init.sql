...
CREATE TABLE `platform_user` (
  ...
  `username` varchar(50) NOT NULL COMMENT '用户名（仅支持英文+数字+下划线，唯一）',
...
CREATE TABLE `admin_operation_log` (
  ...
  `operation` varchar(50) NOT NULL COMMENT '操作类型（login/user_status/recharge/register/pack_create/pack_update/pack_delete/config_update/change_password）',
  `resource_type` varchar(50) DEFAULT NULL COMMENT '资源类型（user/pack/order/config）',
...
-- ----------------------------
-- 13. 存量数据库升级脚本（v1.5.0：新增用户名/邮箱）
-- 已有生产环境执行以下 SQL，切勿重复执行：
-- ----------------------------
-- -- 1) 为存量用户补全 username/email（user{ID} 格式，保证唯一且符合规则）
-- ALTER TABLE `platform_user` ADD COLUMN `username` varchar(50) NULL COMMENT '用户名（仅支持英文+数字+下划线，唯一）' AFTER `phone`;
-- ALTER TABLE `platform_user` ADD COLUMN `email` varchar(100) NULL COMMENT '邮箱（唯一）' AFTER `username`;
-- UPDATE `platform_user` SET `username` = CONCAT('user', `id`) WHERE `username` IS NULL OR `username` = '';
-- UPDATE `platform_user` SET `email` = CONCAT('user', `id`, '@sl.local') WHERE `email` IS NULL OR `email` = '';
-- ALTER TABLE `platform_user` MODIFY `username` varchar(50) NOT NULL;
-- ALTER TABLE `platform_user` MODIFY `email` varchar(100) NOT NULL;
-- ALTER TABLE `platform_user` ADD UNIQUE KEY `uk_username` (`username`);
